...
def filosofo(f):
    try:
        while True:
            ...
            pegaGarfo(f)
            print (termcolor.colored('[+]', 'red'), 'Filosofo ' + termcolor.colored(str(f+1), 'red') + ' pegou os garfos.\r')
            ...
            largaGarfo(f)
            print (termcolor.colored('[-]', 'green'), 'Filosofo '+ termcolor.colored(str(f+1), 'green') + ' largou os garfos.\r')
            # CRIA UM SLEEP(DELAY) ALEATORIO DE 1 A 5 SEGUNDOS PARA O FILOSOFO PENSAR
            print (termcolor.colored('[=]', 'blue'), 'Filosofo ' + termcolor.colored(str(f+1), 'blue') + ' está pensando.\r')
            time.sleep(random.randint(1, 10))
